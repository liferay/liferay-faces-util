...
public class OnDemandTest {
	...
	private static void testThreadSafe(final OnDemandTestResult ON_DEMAND_TEST_RESULT) {
		...
		// Attempt to obtain the test result. If the current thread is blocked, waitingForWhileLoopThread will exit
		// the while loop in OnDemand and the current thread will be unblocked and OnDemand.get() will return true.
		assertTrue(ON_DEMAND_TEST_RESULT, TEST_THREAD);
	}
	...
	private static final class OnDemandTestResult extends OnDemand<Boolean, Thread> {

		@Override
		protected Boolean computeInitialValue(Thread thread) {

			int waitTime = 0;

			// Wait until the passed thread is blocked to prove that at most only 1 thread can access the
			// computeInitialValue() method.
